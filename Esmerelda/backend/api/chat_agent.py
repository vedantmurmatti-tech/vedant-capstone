"""Esmerelda's chat agent: perceive -> reason -> act -> observe -> iterate.

This is a deterministic, rule-based agent over real rows from
storage/esmerelda.db (via queries.py) — there is no LLM call anywhere in
this module. It composes its reply from real records only; when it can't
map a message to anything it can look up, it says so instead of guessing.
...
Loop shape, once per incoming message:
  ...
  act       — each step runs a real query (via queries.py) and, for
              ...
              (see assignment_action_planner.py) to rank/recommend.
...
from . import queries
from .assignment_action_planner import plan_actions, summarize_plan
...
def _match_course(message: str, courses: list[Course]) -> Course | None:
    lower = message.lower()
...
def _perceive(message: str, courses: list[Course]) -> Perception:
    ...
    return Perception(intents=intents or [], course=_match_course(message, courses))
...
def _act_course_lookup(db: Session, course: Course, perception: Perception, ctx: AgentContext) -> None:
    ...
    ctx.reply_parts.append("\n\n".join(parts))
    if "documents" not in perception.intents:
        ctx.follow_ups.append(f"What documents are indexed for {short}?")
...
def _act_deadlines(db: Session, ctx: AgentContext) -> None:
    ...
    if actions:
        ...
        # This agent is stateless (no conversation memory), so a vague
        # follow-up like "the top one" would fall through to the fallback
        # step — name the actual top course instead, which resolves via
        # _match_course on the next turn.
        top_course = course_by_assignment[actions[0].assignment.id]
        ctx.follow_ups.append(f"Tell me more about {top_course.name.split('-')[0].strip()}")
...
def _act_fallback(ctx: AgentContext) -> None:
    ...
    ctx.follow_ups.extend(["What's due this week?", "What can you do?"])
...
def handle_chat_message(db: Session, message: str) -> ChatResponseOut:
    ...
    return ChatResponseOut(
        ...
        courses=list(ctx.courses.values()),
