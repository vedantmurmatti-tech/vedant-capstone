...
with sync_playwright() as p:
    ...
    status1, desc1 = _fetch_assignment_page_details(page, "BASE_URL/with-intro")
    print(f"WITH_INTRO_STATUS:{status1}")
    print(f"WITH_INTRO_DESC:{desc1}")

    status2, desc2 = _fetch_assignment_page_details(page, "BASE_URL/no-intro")
    print(f"NO_INTRO_DESC:{desc2!r}")

    status3, desc3 = _fetch_assignment_page_details(page, "BASE_URL/with-long-intro")
...
try:
    ...
    check("5. save_assignment description-preservation subprocess exited successfully", proc2.returncode == 0)
    if proc2.returncode != 0:
        print(proc2.stdout)
        print(proc2.stderr)
    check(
        "6. A later save_assignment() call with description=None does not erase a previously-saved real "
        "description",
        "DESCRIPTION:'Write a report.'" in proc2.stdout,
    )
    check(
        "7. A later save_assignment() call WITH a real, different description correctly updates it — "
